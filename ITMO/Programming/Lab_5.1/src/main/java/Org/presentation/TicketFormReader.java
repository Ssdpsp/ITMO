...
/**
 * Чтение объектов билета из текущего источника ввода.
 */
public final class TicketFormReader {
    ...
    /**
     * Чтение билета с back/skip/cancel.
     *
     * @return read result
     */
    public ObjectReadResult<TicketDraft> readTicketDraft() {
        String name = null;
     ...
     *
     * @param raw raw value
     * @return parsed boolean when possible
     */
    public Optional<Boolean> parseBoolean(final String raw) {
        if (raw == null) {
    ...
    private ReadFieldResult readField(final InputMode mode, final String prompt, final boolean nullable) {
    ...
    private String errorWithSuggestion(final ValidationResult result) {
        String joined = String.join("; ", result.getErrors());
        return joined + " Введите значение в допустимых пределах.";
    }

    private Integer parseInt(final String raw, final String errorMessage) {
        try {
    ...
    private Double parseDouble(final String raw, final String errorMessage) {
        try {
    ...
    private LocalDateTime parseLocalDateTime(final String raw) {
        try {
    ...
    private TicketType parseTicketType(final String raw) {
    ...
    private static final class ReadFieldResult {
        ...
        private static ReadFieldResult value(final String value) {
        ...
        private static ReadFieldResult failed(final String reason) {
