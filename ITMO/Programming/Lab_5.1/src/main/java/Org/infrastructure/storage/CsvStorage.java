...
import Org.domain.model.TicketType;
import Org.domain.validation.ValidationResult;
import Org.domain.validation.ValidationService;
import Org.infrastructure.logging.AppLogger;

/**
 * CSV хранилище для сбора билетов.
 */
public final class CsvStorage {
     ...
     * @param logger logger
     */
    public CsvStorage(final ValidationService validationService, final AppLogger logger) {
        this.validationService = validationService;
        this.logger = logger;
    }

    /**
     * Загрузка билетов из CSV.
     *
     * @param filePath file path
     * @return load report
     */
    public LoadReport load(final Path filePath) {
        if (filePath == null) {
    ...
    /**
     * Сохранение билетов в CSV.
     *
     * @param filePath file path
     * @param tickets tickets
     * @return save report
     */
    public SaveReport save(final Path filePath, final LinkedHashSet<Ticket> tickets) {
        if (filePath == null) {
    ...
    private Ticket parseTicket(final List<String> values) {
    ...
    private Event parseEvent(
            ...
            final String eventDescriptionRaw
    ) {
    ...
    private String stripUtf8Bom(final String line) {
    ...
    private String toCsvLine(final Ticket ticket) {
    ...
    private String esc(final String value) {
        if (value == null) {
            return "";
        }
    ...
    private long parseLong(final String raw, final String field) {
        try {
    ...
    private int parseInt(final String raw, final String field) {
        try {
    ...
    private double parseDouble(final String raw, final String field) {
        try {
    ...
    private Boolean parseBoolean(final String raw, final String field) {
    ...
    private TicketType parseTicketTypeNullable(final String raw, final String field) {
    ...
    private ZonedDateTime parseZonedDateTime(final String raw, final String field) {
        try {
    ...
    private LocalDateTime parseLocalDateTime(final String raw, final String field) {
        try {
